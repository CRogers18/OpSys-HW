...
static ssize_t dev_write(struct file *filp, const char* buff, size_t len, loff_t* off)
{
    ...
    for(i = 0; i < len; i++)
    {
        ...
        {   
	        ...
	        { // If the first character in the input buffer string is C or F we need to do some checks to see if
	    	  // a UCF input was built.
	            if(buff[i] == 'C' && currentPos != 0){ //If we get in a C and its at the start of the input buffer
	       		...
	       		else if(buff[i] == 'F' && currentPos >= 2){ //If we get in an F at the beginning of the input buffer
	       		...
	       		else{ // If were at the frst index and its not U C or F, insert and move on
	       			mainBuffer[currentPos] = buff[i];
	               	printk(KERN_INFO "[DEBUG] mainBuffer inserted %c\n", mainBuffer[currentPos]);
	               	currentPos++;
	           		bytesWritten++;	
			...
			else{ // If i != 0 and buff[i] != U and every other case. Insert like normal
